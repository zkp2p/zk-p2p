...
template VenmoReceiveEmail(max_header_bytes, max_body_bytes, n, k, pack_size) {
    ...
    var max_email_from_len = (35 \ pack_size + 1) * pack_size;        // Should be 254, but we limit to 35 + pack_size for now
    var max_email_amount_len = (10 \ pack_size + 1) * pack_size;      // Allowing max 7 fig amount + one decimal point + 2 decimal places
    var max_payee_len = (21 \ pack_size + 1) * pack_size;             // 19 digits, 21 + pack_size is safe if Venmo adds more users
...
component main { public [ order_id ] } = VenmoReceiveEmail(1024, 6400, 121, 17, 7);

...
include "./regexes/venmo/venmo_receive_amount.circom";
include "./regexes/venmo/venmo_receive_id.circom";
...
template VenmoReceiveEmail(max_header_bytes, max_body_bytes, n, k, pack_size) {
    ...
    // VENMO RECEIVE PAYEE ID AND PAYER ID REGEX
    ...
    signal (receive_regex_out, payer_regex_reveal[max_body_bytes], payee_regex_reveal[max_body_bytes]) <== VenmoReceiveIdRegex(max_body_bytes)(in_body_padded);
    signal is_found_receive <== IsZero()(receive_regex_out);
    is_found_receive === 0;
    ...
    // TOTAL CONSTRAINTS: 6914376
}

// Args:
// * max_header_bytes = 1024 is the max number of bytes in the header
// * max_body_bytes = 6528 is the max number of bytes in the body after precomputed slice (Need to leave room for >280 char custom message)
// * n = 121 is the number of bits in each chunk of the modulus (RSA parameter)
// * k = 17 is the number of chunks in the modulus (RSA parameter)
// * pack_size = 7 is the number of bytes that can fit into a 255ish bit signal (can increase later)
component main { public [ intent_hash ] } = VenmoReceiveEmail(1024, 6528, 121, 17, 7);
...
async function getArgs() {
  ...
  const email_type = emailTypeArg ? emailTypeArg.split("=")[2] : "test";
...
async function test_generate(writeToFile: boolean = true) {
  const { email_file, email_type, nonce } = await getArgs();
  const email = fs.readFileSync(email_file.trim());
  console.log(email);
  const type = CircuitType[email_type.trim() as keyof typeof CircuitType];
  const gen_inputs = await generate_inputs(email, type, "1", nonce);
  console.log(JSON.stringify(gen_inputs));
  if (writeToFile) {
    const file_dir = email_file.substring(0, email_file.lastIndexOf("/") + 1);
    const filename = nonce ? `${file_dir}/input_${email_file}_${nonce}.json` : `${file_dir}/input_${email_file}.json`;

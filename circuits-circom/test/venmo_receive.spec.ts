...
describe("Venmo send WASM tester", function () {
    ...
    it("Should return the correct hashed offramper id", async () => {
        ...
        const regex_end = regex_start_sub_array.indexOf("34"); // Look for `"` to end the offramper_id which is 34 in ascii
    ...
    it("Should not generate witnesses for a send email", async () => {
        // Generate input with receive email using yarn gen-input:receive:send
        const venmo_path = path.join(__dirname, "../inputs/input_venmo_receive_send.json");
        const jsonString = fs.readFileSync(venmo_path, "utf8");
        const input = JSON.parse(jsonString);
        try {
            ...
            assert.equal(error.message, 'Error: Assert Failed.\nError in template VenmoReceiveEmail_405 line: 76\n');
        }
    });

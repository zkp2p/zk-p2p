...
template MercadoPayeeIdRegex(msg_bytes) {
	...
	for (var i = 0; i < num_bytes; i++) {
		...
		eq[4][i].in[1] <== 58;
		and[5][i] = AND();
		and[5][i].a <== states[i][40];
		and[5][i].b <== eq[4][i].out;
		and[6][i] = AND();
		and[6][i].a <== states[i][47];
		...
		and[56][i].b <== eq[24][i].out;
		and[57][i] = AND();
		and[57][i].a <== states[i][42];
		and[57][i].b <== eq[2][i].out;
		multi_or[12][i] = MultiOR(2);
		multi_or[12][i].in[0] <== and[56][i].out;
		multi_or[12][i].in[1] <== and[57][i].out;
		states[i+1][44] <== multi_or[12][i].out;
		...
		and[63][i] = AND();
		and[63][i].a <== states[i][3];
		and[63][i].b <== eq[23][i].out;
		states[i+1][48] <== and[63][i].out;
		state_changed[i].in[47] <== states[i+1][48];
		and[64][i] = AND();
		and[64][i].a <== states[i][48];
		and[64][i].b <== eq[5][i].out;
		states[i+1][49] <== and[64][i].out;
	...
	for (var i = 0; i < msg_bytes; i++) {
		is_consecutive[msg_bytes-1-i][0] <== states[num_bytes-i][35] * (1 - is_consecutive[msg_bytes-i][1]) + is_consecutive[msg_bytes-i][1];
	...
	for (var i = 0; i < msg_bytes; i++) {
		...
		is_substr0[i][5] <== is_substr0[i][4] + states[i+1][33] * states[i+2][36];
		is_substr0[i][6] <== is_substr0[i][5] + states[i+1][36] * states[i+2][31];

...
template MercadoPayeeIdRegex(msg_bytes) {
	...
	for (var i = 0; i < num_bytes; i++) {
		...
		eq[2][i].in[1] <== 58;
		and[2][i] = AND();
		and[2][i].a <== states[i][43];
		and[2][i].b <== eq[2][i].out;
		and[3][i] = AND();
		and[3][i].a <== states[i][50];
		...
		multi_or[8][i] = MultiOR(13);
		multi_or[8][i].in[0] <== eq[1][i].out;
		...
		multi_or[8][i].in[11] <== eq[22][i].out;
		multi_or[8][i].in[12] <== eq[3][i].out;
		and[44][i].b <== multi_or[8][i].out;
		and[45][i] = AND();
		and[45][i].a <== states[i][37];
		and[45][i].b <== multi_or[8][i].out;
		multi_or[9][i] = MultiOR(2);
		multi_or[9][i].in[0] <== and[44][i].out;
		multi_or[9][i].in[1] <== and[45][i].out;
		states[i+1][37] <== multi_or[9][i].out;
		...
		states[i+1][39] <== multi_or[10][i].out;
		...
		and[64][i] = AND();
		and[64][i].a <== states[i][13];
		and[64][i].b <== eq[25][i].out;
		and[65][i] = AND();
		and[65][i].a <== states[i][29];
		and[65][i].b <== eq[25][i].out;
		and[66][i] = AND();
		and[66][i].a <== states[i][34];
		and[66][i].b <== eq[25][i].out;
		multi_or[13][i] = MultiOR(5);
		multi_or[13][i].in[0] <== and[62][i].out;
		multi_or[13][i].in[1] <== and[63][i].out;
		multi_or[13][i].in[2] <== and[64][i].out;
		multi_or[13][i].in[3] <== and[65][i].out;
		multi_or[13][i].in[4] <== and[66][i].out;
		states[i+1][51] <== multi_or[13][i].out;
	...
	for (var i = 0; i <= num_bytes; i++) {
		final_state_result.in[i] <== states[i][37];
	}
	...
	for (var i = 0; i < msg_bytes; i++) {
		is_consecutive[msg_bytes-1-i][0] <== states[num_bytes-i][37] * (1 - is_consecutive[msg_bytes-i][1]) + is_consecutive[msg_bytes-i][1];
	...
	for (var i = 0; i < msg_bytes; i++) {
		is_substr0[i][0] <== 0;
		is_substr0[i][1] <== is_substr0[i][0] + states[i+1][33] * states[i+2][37];
		is_substr0[i][2] <== is_substr0[i][1] + states[i+1][37] * states[i+2][37];

...
template GarantiPaymentDetailsRegex(msg_bytes) {
	...
	for (var i = 0; i < num_bytes; i++) {
		...
		multi_or[2][i].in[0] <== eq[28][i].out;
		multi_or[2][i].in[1] <== eq[29][i].out;
		...
		and[55][i].b <== multi_or[2][i].out;
		and[56][i] = AND();
		and[56][i].a <== states[i][55];
		and[56][i].b <== multi_or[2][i].out;
		multi_or[3][i] = MultiOR(2);
		multi_or[3][i].in[0] <== and[55][i].out;
		multi_or[3][i].in[1] <== and[56][i].out;
		states[i+1][55] <== multi_or[3][i].out;
	...
	for (var i = 0; i <= num_bytes; i++) {
		final_state_result.in[i] <== states[i][74];
	}
	...
	for (var i = 0; i < msg_bytes; i++) {
		is_consecutive[msg_bytes-1-i][0] <== states[num_bytes-i][74] * (1 - is_consecutive[msg_bytes-i][1]) + is_consecutive[msg_bytes-i][1];
	...
	for (var i = 0; i < msg_bytes; i++) {
		...
		is_substr0[i][2] <== is_substr0[i][1] + states[i+1][75] * states[i+2][76];
		is_substr0[i][3] <== is_substr0[i][2] + states[i+1][76] * states[i+2][77];
		is_substr0[i][4] <== is_substr0[i][3] + states[i+1][77] * states[i+2][1];
	...
	for (var i = 0; i < msg_bytes; i++) {
		is_substr1[i][0] <== 0;
		is_substr1[i][1] <== is_substr1[i][0] + states[i+1][54] * states[i+2][55];
		is_substr1[i][2] <== is_substr1[i][1] + states[i+1][55] * states[i+2][55];

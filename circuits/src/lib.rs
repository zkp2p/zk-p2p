...
mod test {
    ...
    #[test]
    fn test_generated_email_paypal_send() {
        temp_env::with_var(EMAIL_VERIFY_CONFIG_ENV, Some("./configs/test_generated_paypal_send.config"), || {
            let regex_bodyhash_decomposed: DecomposedRegexConfig = serde_json::from_reader(File::open("./test_data/bodyhash_defs.json").unwrap()).unwrap();
            regex_bodyhash_decomposed
                .gen_regex_files(
                    &Path::new("./test_data/bodyhash_allstr.txt").to_path_buf(),
            ...
            regex_body_decomposed
                .gen_regex_files(
                    &Path::new("./test_data/test_generated_paypal_send_body_allstr.txt").to_path_buf(),
                    &[
                        Path::new("./test_data/test_generated_paypal_send_body_substr_0.txt").to_path_buf()
                    ],
                )
                .unwrap();
            let params = DefaultEmailVerifyCircuit::<Fr>::read_config_params();
    ...
    #[cfg(feature = "dev-graph")]
    #[tokio::test]
    async fn test_existing_paypal_send_email() {
        let regex_bodyhash_decomposed: DecomposedRegexConfig = serde_json::from_reader(File::open("./test_data/bodyhash_defs.json").unwrap()).unwrap();
        regex_bodyhash_decomposed
            .gen_regex_files(
                &Path::new("./test_data/bodyhash_allstr.txt").to_path_buf(),
        ...
        regex_body_decomposed
            .gen_regex_files(
                &Path::new("./test_data/test_ex_paypal_send_body_allstr.txt").to_path_buf(),
                &[
                    Path::new("./test_data/test_ex_paypal_send_body_substr_0.txt").to_path_buf()
                ],
            )
            .unwrap();
        let email_bytes = {
            ...
            let mut f = File::open("./build/paypal_send_payment.eml").unwrap();
            let mut buf = Vec::new();
            f.read_to_end(&mut buf).unwrap();
            buf
        };
        ...
        temp_env::with_var(EMAIL_VERIFY_CONFIG_ENV, Some("./configs/test_ex_paypal_send.config"), move || {
            let params = DefaultEmailVerifyCircuit::<Fr>::read_config_params();
            let (canonicalized_header, canonicalized_body, signature_bytes) = canonicalize_signed_email(&email_bytes).unwrap();
            println!("header len\n {}", canonicalized_header.len());
            println!("body len\n {}", canonicalized_body.len());
    ...
    #[test]
    fn test_generated_email_paypal_receive() {
        temp_env::with_var(EMAIL_VERIFY_CONFIG_ENV, Some("./configs/test_generated_paypal_receive.config"), || {
            let regex_bodyhash_decomposed: DecomposedRegexConfig = serde_json::from_reader(File::open("./test_data/bodyhash_defs.json").unwrap()).unwrap();
            regex_bodyhash_decomposed
                .gen_regex_files(
                    &Path::new("./test_data/bodyhash_allstr.txt").to_path_buf(),
            ...
            regex_to_decomposed
                ...
                .unwrap();
            let regex_body_decomposed: DecomposedRegexConfig = serde_json::from_reader(File::open("./test_data/test_generated_paypal_receive_body_defs.json").unwrap()).unwrap();
            regex_body_decomposed
                .gen_regex_files(
                    &Path::new("./test_data/test_generated_paypal_receive_body_allstr.txt").to_path_buf(),
                    &[
                        Path::new("./test_data/test_generated_paypal_receive_body_substr_0.txt").to_path_buf()
                    ],
                )
                .unwrap();
            let params = DefaultEmailVerifyCircuit::<Fr>::read_config_params();
            ...
            let message = concat!(
                ...
                "To: bob@zkemail.com\r\n",
                "\r\n",
                "\r\net=3D\"_blank\"><span>0764479328410520R</span></a></td>=0A                   =\r\n",
                "\r\n"
            )
            .as_bytes();
            let email = parse_mail(message).unwrap();
    ...
    #[cfg(feature = "dev-graph")]
    #[tokio::test]
    async fn test_existing_wise_receive_email() {
        let regex_bodyhash_decomposed: DecomposedRegexConfig = serde_json::from_reader(File::open("./test_data/bodyhash_defs.json").unwrap()).unwrap();
        regex_bodyhash_decomposed
            .gen_regex_files(
                &Path::new("./test_data/bodyhash_allstr.txt").to_path_buf(),
        ...
        regex_to_decomposed
            ...
            .unwrap();
        let regex_body_decomposed: DecomposedRegexConfig = serde_json::from_reader(File::open("./test_data/test_ex_wise_receive_body_defs.json").unwrap()).unwrap();
        regex_body_decomposed
            .gen_regex_files(
                &Path::new("./test_data/test_ex_wise_receive_body_allstr.txt").to_path_buf(),
                &[
                    Path::new("./test_data/test_ex_wise_receive_body_substr_0.txt").to_path_buf()
                ],
            )
            .unwrap();
        let email_bytes = {
            // NOTE: Download a Wise payment received email into the build folder. We don't include it in the repo due to privacy reasons
            let mut f = File::open("./build/transferwise_receive_payment.eml").unwrap();
            let mut buf = Vec::new();
            f.read_to_end(&mut buf).unwrap();
            buf
        };
        ...
        temp_env::with_var(EMAIL_VERIFY_CONFIG_ENV, Some("./configs/test_ex_wise_receive.config"), move || {
            let params = DefaultEmailVerifyCircuit::<Fr>::read_config_params();
            let (canonicalized_header, canonicalized_body, signature_bytes) = canonicalize_signed_email(&email_bytes).unwrap();
            println!("header len\n {}", canonicalized_header.len());
            println!("body len\n {}", canonicalized_body.len());
    ...
    #[cfg(feature = "dev-graph")]
    #[tokio::test]
    async fn test_existing_wise_send_email() {
        let regex_bodyhash_decomposed: DecomposedRegexConfig = serde_json::from_reader(File::open("./test_data/bodyhash_defs.json").unwrap()).unwrap();
        regex_bodyhash_decomposed
            .gen_regex_files(
                &Path::new("./test_data/bodyhash_allstr.txt").to_path_buf(),
        ...
        let email_bytes = {
            // NOTE: Download a Wise payment send email into the build folder. We don't include it in the repo due to privacy reasons
            let mut f = File::open("./build/transferwise_send_payment.eml").unwrap();
            let mut buf = Vec::new();
            f.read_to_end(&mut buf).unwrap();
            buf
        };
        ...
        temp_env::with_var(EMAIL_VERIFY_CONFIG_ENV, Some("./configs/test_ex_wise_send.config"), move || {
            let params = DefaultEmailVerifyCircuit::<Fr>::read_config_params();
            let (canonicalized_header, canonicalized_body, signature_bytes) = canonicalize_signed_email(&email_bytes).unwrap();
            println!("header len\n {}", canonicalized_header.len());
            println!("body len\n {}", canonicalized_body.len());
